class TextSearch {
  findSelectedPatterns(terms, text){
    //Primeiro vou achar os termos de maior tamanho
    let result = [];
    let match;

    this.terms = terms.sort( (a, b) => b.length - a.length);//Considera o termo de maior tamanho//Ordenar pela quantidade de palavras
    this.text = text;

    this.terms.map( term => {
      let regex = new RegExp(`\\b${term}\\b`, 'g');
      while((match = regex.exec(this.text)) != null){
        result.push(term); //Adiciono a palavra localizada em um array
...
// console.log(`Texto : ${textSearch.text}`, `Termos: ${result.foundWords}`)
